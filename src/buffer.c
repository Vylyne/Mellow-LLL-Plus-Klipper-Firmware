...
#define KEY1_PIN      GPIO('B', 13)  // KEY1 后退 (retract)
#define KEY2_PIN      GPIO('B', 12)  // KEY2 前进 (feed)
...
#define EXT1_PIN      GPIO('A', 2)   // EXTENSION_PIN1
#define EXT2_PIN      GPIO('A', 3)   // EXTENSION_PIN2
...
// État
// ------------------------------------------------------------------
enum { ST_STOP = 0, ST_FORWARD, ST_BACK };

static struct gpio_in hall1, hall2, hall3, fil, key1, key2, front_sig, back_sig;
...
tmc_configure(void)
{
    ...
    tmc_write(TMC_GCONF, GCONF_VAL);        tmc_delay_ms(2);
    tmc_write(TMC_CHOPCONF, CHOPCONF_VAL);  tmc_delay_ms(2);
    tmc_write(TMC_IHOLD_IRUN, IHOLD_IRUN_VAL); tmc_delay_ms(2);
    tmc_write(TMC_PWMCONF, PWMCONF_VAL);    tmc_delay_ms(2);
    tmc_write(TMC_VACTUAL, 0);              tmc_delay_ms(2);
    last_state = ST_STOP;
}
...
buffer_init(void)
{
    ...
    key1  = gpio_in_setup(KEY1_PIN, 0);
    key2  = gpio_in_setup(KEY2_PIN, 0);
...
apply_state(uint8_t state)
{
    ...
    case ST_FORWARD:
        ...
        if (last_state == ST_BACK) { tmc_write(TMC_VACTUAL, 0); tmc_delay_ms(2); }
    ...
    case ST_BACK:
        gpio_out_write(en_out, 0);
        if (last_state == ST_FORWARD) { tmc_write(TMC_VACTUAL, 0); tmc_delay_ms(2); }
...
process_buttons(void)
{
    ...
    uint8_t k1 = gpio_in_read(key1);    // 1=relâché, 0=pressé
    uint8_t k2 = gpio_in_read(key2);

    // détection de fronts (remplace key_it_callback CHANGE)
    if (key1_prev && !k1) { key1_press_t = now; key1_cnt++; key1_held = 1; }
    else if (!key1_prev && k1) {
        if (now - key1_press_t <= MS_TICKS(500)) { key1_rel = 1; key1_rel_t = now; }
        else key1_cnt = 0;
        key1_held = 0;
    }
    key1_prev = k1;
    if (key2_prev && !k2) { key2_press_t = now; key2_cnt++; key2_held = 1; }
    else if (!key2_prev && k2) {
        if (now - key2_press_t <= MS_TICKS(500)) { key2_rel = 1; key2_rel_t = now; }
        else key2_cnt = 0;
        key2_held = 0;
    }
    key2_prev = k2;
    ...
    if (inform && now - inform_t >= MS_TICKS(3000)) {
        inform = 0; gpio_out_write(ext2, 1); gpio_out_write(ext1, 0);
    }
    // KEY1 clic court : 1x -> EXT2 LOW + inform ; 2x+ -> is_error (pause)
    if (key1_rel && now - key1_rel_t > MS_TICKS(500)) {
        key1_rel = 0;
        if (key1_cnt == 1) { gpio_out_write(ext2, 0); inform_t = now; inform = 1; is_error = 0; }
        else if (key1_cnt >= 2) is_error = 1;
        key1_cnt = 0;
    }
    // KEY2 clic court : 1x -> EXT1 HIGH + inform ; 2x+ -> is_error (pause)
    if (key2_rel && now - key2_rel_t > MS_TICKS(500)) {
        key2_rel = 0;
        if (key2_cnt == 1) { gpio_out_write(ext1, 1); inform_t = now; inform = 1; is_error = 0; }
        else if (key2_cnt >= 2) is_error = 1;
        key2_cnt = 0;
    }
    ...
    if (back_act) {                          // priorité BACK (comme motor_control)
        if (manual != 2) {
            gpio_out_write(en_out, 0);
            tmc_write(TMC_VACTUAL, 0); tmc_delay_ms(2);
            tmc_write(TMC_GCONF, GCONF_VAL); tmc_delay_ms(2);    // shaft=0 (BACK)
            tmc_write(TMC_VACTUAL, (uint32_t)VACTUAL_MAG);
            manual = 2; last_state = ST_BACK;
        }
        return 1;
    }
    if (fwd_act) {
        if (manual != 1) {
            gpio_out_write(en_out, 0);
            tmc_write(TMC_VACTUAL, 0); tmc_delay_ms(2);
            tmc_write(TMC_GCONF, GCONF_VAL | GCONF_SHAFT); tmc_delay_ms(2);  // shaft=1 (FWD)
            tmc_write(TMC_VACTUAL, (uint32_t)VACTUAL_MAG);
            manual = 1; last_state = ST_FORWARD;
        }
        return 1;
    }
    if (manual) {                            // fin du maintien -> stop
        tmc_write(TMC_VACTUAL, 0); tmc_delay_ms(2);
        gpio_out_write(en_out, 1);
        last_state = ST_STOP;
        if (manual == 2) is_error = 1;       // standalone : is_error=true après BACK long-press
        manual = 0;
        return 1;
    }
    return 0;
...
buffer_task(void)
{
    if (!tmc_ready) {                    // 1er appel : timer prêt -> config TMC
        tmc_configure();
        tmc_ready = 1;
        return;
    }
    ...
    if (is_error) {                      // pause (double-clic / timeout) -> stop
        apply_state(ST_STOP);
        return;
    }
    ...
    uint8_t state;
    if (gpio_in_read(hall3))        state = ST_FORWARD;   // pos1
    else if (gpio_in_read(hall2))   state = ST_STOP;      // pos2
    else if (gpio_in_read(hall1))   state = ST_BACK;      // pos3
    else                            state = last_state;   // aucun hall : garde l'état

    // Sécurité : feed continu > 60s -> erreur (buffer.cpp front_time>timeout)
    if (state == ST_FORWARD) {
        if (!is_feeding) { is_feeding = 1; feed_start_time = timer_read_time(); }
