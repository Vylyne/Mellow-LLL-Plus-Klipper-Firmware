...
class FilamentWatcher:
    def __init__(self, config):
        ...
        self.confirm_window = config.getfloat("confirm_window", 5.0, above=0.0)
        self.detection_length = config.getfloat("detection_length", 3.5, above=0.0)
        ...
        self.printer.register_event_handler(
            "idle_timeout:printing", self._reset_pending
        )
    ...
    def _poll(self, eventtime):
        ...
        if in_grace:
            self.distance_since_pulse = (
                0.0  # reversal still absorbing backlash - don't count it
            )
        else:
    ...
    def _trigger_runout(self, eventtime):
        ...
        if (
            self.jam_pending_since is not None
            and (eventtime - self.jam_pending_since) <= self.confirm_window
        ):
            self.jam_pending_since = None
            self.escalated = True
